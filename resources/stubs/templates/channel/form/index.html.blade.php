<html>
    ...
    <body>
        <div>
            @if(!empty($segment_2))
                @forelse($exp->channel->entries(['channel' => "<?= $channel ?>", 'url_title' => $segment_2]) as $entry)
            ...
            @php
                $form = $exp->channel->form(['channel' => "<?=$channel?>", 'url_title' => $segment_2, 'error_handling' => "message"]);
            @endphp
            {!! $form->open() !!}
                <fieldset>
                ...
                <?php foreach ($fields as $field) : ?>
                    ...
                    <fieldset class="element-wrapper <?=$field['field_type']?>-wrap">
                        ...
                        {!! $form->fields()->get('<?=$field['field_name']?>')['input'] !!}
